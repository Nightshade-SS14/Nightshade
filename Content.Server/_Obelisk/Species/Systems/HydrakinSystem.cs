...
public sealed partial class HydrakinSystem : EntitySystem
{
    ...
    private void OnCoolOffDoAfter(Entity<HydrakinComponent> ent, ref CoolOffDoAfterEvent args)
    {
        ...
        var dT = -(ent.Comp.CoolOffCoefficient * temperatureComponent.Temperature);
        var C_h = temperatureComponent.HeatCapacity;
        var Q = C_h * dT;

        _temp.ChangeHeat((ent.Owner, temperatureComponent), Q, true);
        args.Handled = true;
    }
}
